...
bool intersection(
	...
	if (ax >= ay && ax >= az) {
		...
		point.x = -(point.y * plane1.norm.y + point.z * plane1.norm.z + plane1.dist)
				/ plane1.norm.x;
	} else if (ay >= az) {
		...
		point.y = -(point.x * plane1.norm.x + point.z * plane1.norm.z + plane1.dist)
				/ plane1.norm.y;
	} else {
		...
		point.z = -(point.y * plane1.norm.y + point.x * plane1.norm.x + plane1.dist)
				/ plane1.norm.z;
	}
	return true;
}

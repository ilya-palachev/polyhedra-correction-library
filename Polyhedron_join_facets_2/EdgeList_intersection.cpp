...
void EdgeList::find_edge_for_vertex(int v, int& v0, int& v1, int& next_f) {
    int i;
    for (i = 0; i < num; ++i) {
        if (edge0[i] == v || edge1[i] == v) {
            v0 = edge0[i];
            v1 = edge1[i];
            next_f = next_facet[i];
            return;
    ...
    printf("find_edge_for_vertex : Error. Cannot find vertex %d in edgelist %d\n", v, id);
    v0 = -1;
    v1 = -1;
}

void EdgeList::get_next_edge(Plane iplane, int& v0, int& v1, int& i0, int& i1, int& next_f, int& next_d, bool ifSpecialFacet) {
    int i, tmp, i_next;
    int id0;
    int id1;
    int sign0;
    ...
    if (ifSpecialFacet) {
        ...
        if (v0 == v1 && sign0 == 0) {
            v0 = index[(id0 + nv + 1) % nv];
            sign0 = poly->signum(poly->vertex[v0], iplane);
            next_f = index[nv + 1 + id0];
            if (sign0 == 0)
                v1 = v0;
            poly->edge_list[next_f].find_edge_for_vertex(v0, v0, v1, next_f);
            return;
        } else {
            printf("get_next_edge : I DON'T KNOW WHAT TO DO!!!\n");
            return;
        }
        
    } else if (num < 1 && fabs(err) <= 0.0000000000000001) {
...
void EdgeList::get_next_edge(Plane iplane, int& v0, int& v1, int& next_f, int& next_d, bool ifSpecialFacet) {
    int i0, i1;
    get_next_edge(iplane, v0, v1, i0, i1, next_f, next_d, ifSpecialFacet);
}

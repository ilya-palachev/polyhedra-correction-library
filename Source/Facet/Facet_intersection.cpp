		...
		do {
			...
			if (i0 == i1) {
				...
				if (sign0 == 1) {
					i_curr = i0;
					sign_curr = sign0;
					i_step = (i1 + 1) % numVertices == i0 ?
							1 : -1;
				} else {
					i_curr = i1;
					sign_curr = sign0;
					i_step = (i0 + 1) % numVertices == i1 ?
							1 : -1;
				}
			}
...
void Facet::find_and_replace_vertex(
		int from,
		int to) {
...
void Facet::find_and_replace_facet(
		int from,
		int to) {

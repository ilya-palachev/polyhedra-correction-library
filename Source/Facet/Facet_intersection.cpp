...
void min3scalar(double s, double& s0, double& s1, double &s2, int i, int& i0,
		int& i1, int& i2)
{
...
//        printf("===Facet::prepare_edge_list : cannot prepare list for facet %d because planes are parallel or equal...\n", id);
		...
		for (i = 0; i < numVertices; ++i)
		{
			...
			{
				el->add_edge(indVertices[i], indVertices[i], i, i,
						indVertices[numVertices + 1 + i], 1, (double) i);
			}
			else if (sign_curr == 0 && sign_next == 0)
			{
				el->add_edge(indVertices[i], indVertices[i], i, i, id, 1,
						(double) i);
			}
	...
	for (i = 0; i < numVertices; ++i)
	{
		...
		if (sign_curr * sign_next == -1)
		{
			...
			scalar = dir * (v_intrsct - point);
			min3scalar(scalar, s0, s1, s2, i, i0, i1, i2);
			el->add_edge(indVertices[i], indVertices[i_next], i, i_next,
					scalar);
			++n_intrsct;
		}
	}
	...
	for (i = 0; i < numVertices; ++i)
	{
		...
		if (sign_curr == 0 || sign_curr * sign_next == -1)
		{
					...
					)
			{
				...
				if (sign_curr == 0)
					next_f =
							parentPolyhedron->vertexInfos[indVertices[i_curr]].intersection_find_next_facet(
									iplane, id);
				else
				{
			...
			if (sign_curr * sign_next == -1)
				el->search_and_set_info(indVertices[i_curr],
						indVertices[i_next], next_d, next_f);
			else
				el->search_and_set_info(indVertices[i_curr],
						indVertices[i_curr], next_d, next_f);
		}
...
//        return true; //2012-03-10
	...
	while (nintrsct > 0)
	{
		...
		do
		{
			...
			do
			{
				curr_component.add_edge(indVertices[i_curr],
						indVertices[i_curr], id);
				//				fprintf(stdout,
				//						"\tДобавлено ребро : %d %d\n",

...
private:
	...
	enum
	{
		/** Primal problem: minimize ||h - h0|| subject to Q h >= 0 */
		PROBLEM_PRIMAL,
...
public:
	...
	/**
	 * Gives Ipopt the information about the size of the problem (and hence, the
	 * size of arrays it needs to allocate).
	 *
	 ...
	 *
	 * @param index style:	(out), the numbering style used for row/col entries
	 ...
	 *
	 * @param n		(in), the number of variables in the problem (dimension of
	 * 				x).
	 * @param x_l	(out) the lower bounds x^{L} for x.
	 * @param x_u	(out) the upper bounds x^{U} for x.
	 * @param m		(in), the number of constraints in the problem (dimension of
	 * 				g(x)).
	 ...
	 * 						(dimension of x).
	 * @param init_x		(in), if true, this method must provide an initial
	 * 						value for x.
	 * @param x				(out), the initial values for the primal variables,
	 * 						x.
	 * @param init_z		(in), if true, this method must provide an initial
	 * 						value for the bound multipliers z L and zU .
	 * @param z_L			(out), the initial values for the bound multipliers,
	 * 						z^{L} .
	 * @param z_U			(out), the initial values for the bound multipliers,
	 * 						z^{U} .
	 * @param m				(in), the number of constraints in the problem
	 * 						(dimension of g(x)).
	 * @param init_lambda	(in), if true, this method must provide an initial
	 * 						value for the constraint multipliers, λ.
	 ...
	 * @param new_x			(in), false if any evaluation method was previously
	 ...
	 *
	 * @param n			(in), the number of variables in the problem (dimension
	 * 					of x).
	 * @param x			(in), the values for the primal variables, x, at which
	 *					\nabla f (x) is to be evaluated.
	 * @param new_x		(in), false if any evaluation method was previously
	 * 					called with the same values in x, true otherwise.
	 * @param grad_f	(out) the array of values for the gradient of the
	 * 					objective function (\nabla f (x)).
	 */
	 ...
	 * @param m			(in), the number of constraints in the problem
	 * 					(dimension of g(x)).
	 * @param g			(out) the array of constraint function values, g(x).
	 */
	virtual bool eval_g(Index n, const Number* x, bool new_x, Index m,
			Number* g);

	/**
	 * Returns either the sparsity structure of the Jacobian of the constraints,
	 ...
	 * @param m:			(in), the number of constraints in the problem
	 * 						(dimension of g(x)).
	 * @param n_ele_jac		(in), the number of nonzero elements in the Jacobian
	 * 						(dimension of iRow, jCol, and values).
	 * @param iRow			(out), the row indices of entries in the Jacobian of
	 * 						the constraints.
	 * @param jCol			(out), the column indices of entries in the Jacobian
	 * 						of the constraints.
	 * @param values		(out), the values of the entries in the Jacobian of
	 * 						the constraints.
	 */
	 ...
	 * or the values of the Hessian of the Lagrangian (9) for the given values
	 * for x, σ_{f} , and λ.
	 *
	 * @param n				(in), the number of variables in the problem
	 * 						(dimension of x).
	 ...
	 * @param new_x			(in), false if any evaluation method was previously
	 * 						called with the same values in x, trueotherwise.
	 * @param obj_factor	(in), factor in front of the objective term in the
	 * 						Hessian, σf .
	 * @param m				(in), the number of constraints in the problem
	 * 						(dimension of g(x)).
	 * @param lambda		(in), the values for the constraint multipliers, λ,
	 ...
	 * 						otherwise.
	 * @param n_ele_hess	(in), the number of nonzero elements in the Hessian
	 * 						(dimension of iRow, jCol, and values).

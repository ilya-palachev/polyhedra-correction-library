...
VertexGrouper::VertexGrouper() : PCorrector()
{
	DEBUG_START;
	DEBUG_END;
}

VertexGrouper::VertexGrouper(PolyhedronPtr p) : PCorrector(p)
{
	DEBUG_START;
	DEBUG_END;
}

VertexGrouper::VertexGrouper(Polyhedron *p) : PCorrector(p)
{
	DEBUG_START;
	DEBUG_END;
}
...
int VertexGrouper::grouptInnerPair(int fid, int id0, int id1, int id2, int id3, double *A, double *b)
{
	...
	if (b[0] > 0. && b[0] < 1. && b[1] > 0. && b[1] < 1.)
	{
		...
		for (i = 0; i < polyhedron->numFacets; ++i)
		{
			...
			if (pos1 != -1 && pos2 != -1)
			{
				DEBUG_PRINT("\tОбе вершины найдены в грани %d. Из нее "
							"удаляется вершина %d\n",
							i, id2);
			...
			{
				DEBUG_PRINT("\tВершина %d найдена в грани %d. Оставляем грань "
							"без изменения\n",
							id1, i);
			}
			else if (pos2 != -1)
			{
				DEBUG_PRINT("\tВершина %d найдена в грани %d. Заменим здесь "
							"вершину %d на вершину %d\n",
							id2, i, id2, id1);

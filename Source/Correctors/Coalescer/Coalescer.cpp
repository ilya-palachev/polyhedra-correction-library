...
Coalescer::Coalescer() : PCorrector(), plane(), coalescedFacet()
{
	DEBUG_START;
	DEBUG_END;
}

Coalescer::Coalescer(PolyhedronPtr p) : PCorrector(p), plane(), coalescedFacet()
{
	DEBUG_START;
	DEBUG_END;
}

Coalescer::Coalescer(Polyhedron *p) : PCorrector(p), plane(), coalescedFacet()
{
	DEBUG_START;
	DEBUG_END;
}
...
void Coalescer::run(int n, int *fid)
{
	...
	DEBUG_PRINT("III ). Дополнительная предобработка многогранника");
	polyhedron->preprocessAdjacency();
	polyhedron->facets[fid[0]].my_fprint_all(stdout, *polyhedron.get());
...
int Coalescer::buildIndex(int fid0, int fid1)
{
	...
	// 2. Найдем последнюю против часовой стрелки (для 1-й грани) общую точку
	// граней
	...
	//    многогранника
	std::vector<bool> is(polyhedron->numVertices);
	std::vector<bool> del(polyhedron->numVertices);
	...
	coalescedFacet = Facet(fid0, nv, plane, index, true);
...
int Coalescer::buildIndex(int n, int *fid)
{
	...
	std::vector<int> index(3 * nv_safe + 1);
	std::vector<int> nfind(polyhedron->numVertices);
	for (i = 0; i < polyhedron->numVertices; ++i)
	{
		nfind[i] = 0;
	}
	...
	for (i = 0; i < n; ++i)
	{
		...
		polyhedron->facets[fid[i]].my_fprint_all(stdout, *polyhedron.get());
	}
	...
	coalescedFacet = Facet(fid[0], nv, polyhedron->facets[fid[0]].plane, index, true);
	...
	polyhedron->facets[fid[0]].my_fprint_all(stdout, *polyhedron);
...
void Coalescer::calculatePlane(int n, int *fid)
{
	...
	std::vector<int> index = coalescedFacet.indVertices;
	int nv = coalescedFacet.numVertices;
	...
	{
		DEBUG_PRINT("Warning. Cannot define direction of facet (%d positive "
					"and %d negative)",
					n - ninvert, n);
	}
	DEBUG_PRINT("За перемену направления плоскости проголосовало %d граней, а "
				"против - %d граней",
				ninvert, n - ninvert);
	...
	DEBUG_PRINT("Caluclated plane: (%.16lf)x  +  (%.16lf)y  + (%.16lf)z  +  "
				"(%.16lf)  =  0",
...
void Coalescer::rise(int fid0)
{
	...
	while (ndown > 0)
	{
		...
		polyhedron->facets[fid0].my_fprint_all(stdout, *polyhedron.get());
		...
		// расстояние
		DEBUG_PRINT("\t2. 1). Находим, по какой вершине нужно шагать, и "
					"минимальное расстояние");
		imin = riseFind(fid0);

		if (imin == -1)
		{
			ERROR_PRINT("join_facets_rise : Ошибка. Не удалось найти вершину "
						"(imin = -1)");
			DEBUG_END;
			return;
		}
...
int Coalescer::riseFind(int fid0)
{
	...
	for (i = 0; i < nv; ++i)
	{
		...
		//Написано 2012-03-31 для решения проблемы с треугольными соседними
		//гранями
		...
		if (polyhedron->facets[fr1].numVertices == 3)
		{
			...
			--i;
			polyhedron->facets[fid0].my_fprint_all(stdout, *polyhedron.get());
			polyhedron->facets[fr2].my_fprint_all(stdout, *polyhedron.get());
			//            continue;
		}
...
double Coalescer::riseFindStep(int fid0, int i)
{
	...
	if (d < 0)
	{
		...
		polyhedron->facets[fl1].my_fprint_all(stdout, *polyhedron.get());
		polyhedron->facets[fr1].my_fprint_all(stdout, *polyhedron.get());
	}
...
void Coalescer::risePoint(int fid0, int imin)
{
	...
	else
	{
		...
		//            // Если точка пересечения лежит выше плоскости:
		//            DEBUG_PRINT("левая точка пересечения лежит выше
		//            плоскости"); intersection(plane, pl1, pr1, v1);
		//        } else
		ifjoin = true;
	}
	// Найдем проекцию перемещения точки на нормаль плоскости:
	...
	else
	{
		...
		//            // Если точка пересечения лежит выше плоскости:
		//            DEBUG_PRINT("правая точка пересечения лежит выше
		//            плоскости"); intersection(plane, pl1, pr1, v2);
		//        } else
		ifjoin = true;
	}
	// Найдем проекцию перемещения точки на нормаль плоскости:
	...
	if ((d1 <= d2 && d1 > 0) || (d1 > 0 && d2 <= 0))
	{
		...
		if (ifjoin)
		{
			...
			if (deg_imin_big && deg_irep_big)
			{
				...
				polyhedron->facets[fid0].update_info(*polyhedron);
				polyhedron->facets[fl2].update_info(*polyhedron);
				polyhedron->facets[fl1].update_info(*polyhedron);
				polyhedron->facets[fr1].update_info(*polyhedron);
			}
	...
	else if ((d2 < d1 && d2 > 0) || (d2 > 0 && d1 <= 0))
	{
		...
		if (ifjoin)
		{
			...
			if (deg_imin_big && deg_irep_big)
			{
				...
				//Удалить imin из главной грани fid0 и заменить irep на v2; и
				//исправить грань
				index[irep] = ind_new;
				polyhedron->facets[fid0].remove(imin);
				...
				polyhedron->facets[fl1].update_info(*polyhedron);
				polyhedron->facets[fr1].update_info(*polyhedron);
				polyhedron->facets[fr2].update_info(*polyhedron);
			}
			else if (deg_imin_big && !deg_irep_big)
			{
			...
			else if (!deg_imin_big && !deg_irep_big)
			{
				...
				//Заменить вершину imin из грани fl1 на вершину irep и исправить
				//грань
				nnv = polyhedron->facets[fl1].numVertices;
				pos = index[2 * nv + 1 + (nv + imin - 1) % nv];
				pos = (nnv + pos - 1) % nnv;
...
int Coalescer::appendVertex(Vector3d &vec)
{
	...
	polyhedron->vertices.push_back(vec);
	polyhedron->numVertices += 1;

...
GlobalShadowCorrector::GlobalShadowCorrector() :
	...
	edgeData(NULL),
	contourData(NULL),
	parameters({METHOD_CORRECTOR_DEFAULT, EPS_LOOP_STOP_DEFAULT, DELTA_GRADIENT_STEP_DEFAULT}),
	facetsNotAssociated(),
	gradient(NULL),
...
GlobalShadowCorrector::GlobalShadowCorrector(PolyhedronPtr p, ShadowContourDataPtr scd,
...
GlobalShadowCorrector::GlobalShadowCorrector(Polyhedron *p, ShadowContourDataPtr scd,
											 GSCorrectorParameters *_parameters) :
	PCorrector(p),
	edgeData(),
	contourData(scd),
...
void GlobalShadowCorrector::runCorrection()
{
	...
	else
	{
		...
		GSCorrectorStatus *status = new GSCorrectorStatus[NUMBER_METHOD_CORRECTOR];
		...
		for (int iMethod = 0; iMethod < NUMBER_METHOD_CORRECTOR; ++iMethod)
		{
			...
			PRINT("%le \t|\t %d \n", status[iMethod].valueCausedExit, status[iMethod].numIterations);
		}
...
GSCorrectorStatus GlobalShadowCorrector::repairAndRun(MethodCorrector method, Plane *planesInitial)
{
	DEBUG_START;
	for (int iPlane = 0; iPlane < polyhedron->numFacets; ++iPlane)
	{
...
GSCorrectorStatus GlobalShadowCorrector::runCorrectionDo()
{
	...
	for (std::list<int>::iterator iter = facetsNotAssociated.begin(); iter != facetsNotAssociated.end(); ++iter)
	{
		DEBUG_PRINT("%d", *iter);
	}
#endif
	...
	while (error > parameters.epsLoopStop)
	{
		...
		{
			DEBUG_PRINT("Plane[%d]: (%lf, %lf, %lf, %lf) --> (%lf, %lf, %lf, %lf)", *itFacet,
						prevPlanes[*itFacet].norm.x, prevPlanes[*itFacet].norm.y, prevPlanes[*itFacet].norm.z,
						prevPlanes[*itFacet].dist, polyhedron->facets[*itFacet].plane.norm.x,
						polyhedron->facets[*itFacet].plane.norm.y, polyhedron->facets[*itFacet].plane.norm.z,
						polyhedron->facets[*itFacet].plane.dist);

			double oneMovement = qmod(prevPlanes[*itFacet].norm - polyhedron->facets[*itFacet].plane.norm);
			oneMovement += (prevPlanes[*itFacet].dist - polyhedron->facets[*itFacet].plane.dist) *
						   (prevPlanes[*itFacet].dist - polyhedron->facets[*itFacet].plane.dist);
		...
		if (!(parameters.methodName == METHOD_CONJUGATE_GRADIENT && iMinimizedDimension == 0) &&
	...
	for (int iEdge = 0; iEdge < edgeData->numEdges; ++iEdge)
	{
		...
		if (f0 < 0 || f0 >= polyhedron->numFacets)
		{
			ERROR_PRINT("Facet id f0 = %d is out of bounds 0 <= i < %d. It "
						...
						"with info f0 = %d, f1 = %d",
						f0, polyhedron->numFacets, iEdge, edge->v0, edge->v1, f0, f1);
			ASSERT(0);
			DEBUG_END;
		...
		if (f1 < 0 || f1 >= polyhedron->numFacets)
		{
			ERROR_PRINT("Facet id f1 = %d is out of bounds 0 <= i < %d. It "
						"happened during processing the edge #%d = [%d, %d] "
						"with info f0 = %d, f1 = %d",
						f1, polyhedron->numFacets, iEdge, edge->v0, edge->v1, f0, f1);
			ASSERT(0);
			DEBUG_END;
			return DEFAULT_ERROR_FOR_DOUBLE_FUNCTIONS;
		}
		...
		for (std::list<EdgeContourAssociation>::const_iterator itCont = edge->assocList.begin();
			 itCont != edge->assocList.end(); ++itCont)
		{
			...
			double denominator = (planePrev0.norm - planePrev1.norm) * planeOfProjection.norm;
...
void GlobalShadowCorrector::shiftCoefficients(double delta)
{
		 ...
		 ++itFacet, ++iFacetLocal)
	{
		...
		Plane deltaPlane = Plane(
			Vector3d(gradient[4 * iFacetShifted], gradient[4 * iFacetShifted + 1], gradient[4 * iFacetShifted + 2]),
		...
		norm_L2 += abs_a * abs_a + abs_b * abs_b + abs_c * abs_c + abs_d * abs_d;
...
double GlobalShadowCorrector::findOptimalDelta(double deltaMax)
{
	...
	double leftChecker = rightBound - GOLDEN_RATIO_RECIPROCAL * (rightBound - leftBound);
	double rightChecker = leftBound + GOLDEN_RATIO_RECIPROCAL * (rightBound - leftBound);
	while (rightBound - leftBound > INTERVAL_PRECISION)
	{
		DEBUG_PRINT("[%lf   (%lf   %lf)   %lf]", leftBound, leftChecker, rightChecker, rightBound);
		double leftValue = calculateFunctional(leftChecker);
		double rightValue = calculateFunctional(rightChecker);
		if (leftValue > rightValue)
		{
			DEBUG_PRINT("left = %lf > %lf = right, eliminating left", leftValue, rightValue);
			leftBound = leftChecker;
			leftChecker = rightChecker;
			rightChecker = leftBound + GOLDEN_RATIO_RECIPROCAL * (rightBound - leftBound);
		}
		else
		{
			DEBUG_PRINT("left = %lf < %lf = right, eliminating right", leftValue, rightValue);
			rightBound = rightChecker;
			rightChecker = leftChecker;
			leftChecker = rightBound - GOLDEN_RATIO_RECIPROCAL * (rightBound - leftBound);
		}
	}
...
void GlobalShadowCorrector::runCorrectionIteration()
{
	...
	case METHOD_GRADIENT_DESCENT_FAST:
		...
		break;
	case METHOD_CONJUGATE_GRADIENT:
		MAIN_PRINT("iMinimizationLevel = %d, iMinimizedDimension = %d", iMinimizationLevel, iMinimizedDimension);
		if (iMinimizedDimension != 0)
		{
			...
			if (iMinimizedDimension != 1)
			{
				for (int iDimension = 0; iDimension < dim; ++iDimension)
				{
					gradient[iDimension] -= omega * gradientPrevious[iDimension];
				}
			}
			deltaOptimal = findOptimalDelta(parameters.deltaGradientStep);
			MAIN_PRINT("deltaOptimal = %lf", deltaOptimal);
...
void GlobalShadowCorrector::calculateGradient()
{
		 ...
		 ++itFacet, ++iFacetLocal)
	{
		...
		DEBUG_PRINT("planePrevThis: (%lf) x + (%lf) y + (%lf) z + (%lf) = 0", planePrevThis.norm.x,
					planePrevThis.norm.y, planePrevThis.norm.z, planePrevThis.dist);
		...
		for (int iEdge = 0; iEdge < nv; ++iEdge)
		{
			...
			DEBUG_PRINT("planePrevNeighbour: "
						"(%lf) x + (%lf) y + (%lf) z + (%lf) = 0",
						planePrevNeighbour.norm.x, planePrevNeighbour.norm.y, planePrevNeighbour.norm.z,
						planePrevNeighbour.dist);

			DEBUG_PRINT("iFacet = %d, iFacetNeighbour = %d", iFacet, iFacetNeighbour);
			...
			DEBUG_PRINT("Searching edge: [%d, %d] ; Found edge: [%d, %d]", v0, v1, edge->v0, edge->v1);
			...
			if (edge == edgeData->edges.end())
			{
				...
				for (int iEdge = 0; iEdge < edgeData->numEdges; ++iEdge)
				{
					...
					{
						DEBUG_PRINT("It presents: [%d, %d]", edgeDumped->v0, edgeDumped->v1);
					}
					++edgeDumped;
				}
			...
			ASSERT((edge->v0 == v0 && edge->v1 == v1) || (edge->v0 == v1 && edge->v1 == v0));
				 ...
				 itCont != edge->assocList.end(); ++itCont)
			{
				...
				DEBUG_PRINT("Processing association with contour #%d", curContour);

				double enumerator = -planePrevNeighbour.norm * planeOfProjection.norm;
				double denominator = ((planePrevThis.norm - planePrevNeighbour.norm) * planeOfProjection.norm);

				DEBUG_PRINT("enumerator = %le, denominator = %le", enumerator, denominator);
				...
				gradient[i_ak] += ak * coeff_ak_ak + bk * coeff_ak_bk + ck * coeff_ak_ck + dk * coeff_ak_dk;
				...
				gradient[i_bk] += ak * coeff_bk_ak + bk * coeff_bk_bk + ck * coeff_bk_ck + dk * coeff_bk_dk;
				...
				gradient[i_ck] += ak * coeff_ck_ak + bk * coeff_ck_bk + ck * coeff_ck_ck + dk * coeff_ck_dk;
				...
				gradient[i_dk] += ak * coeff_dk_ak + bk * coeff_dk_bk + ck * coeff_dk_ck + dk * coeff_dk_dk;
				...
				gradient[i_ak] += an * coeff_ak_an + bn * coeff_ak_bn + cn * coeff_ak_cn + dn * coeff_ak_dn;
				...
				gradient[i_bk] += an * coeff_bk_an + bn * coeff_bk_bn + cn * coeff_bk_cn + dn * coeff_bk_dn;
				...
				gradient[i_ck] += an * coeff_ck_an + bn * coeff_ck_bn + cn * coeff_ck_cn + dn * coeff_ck_dn;
				...
				gradient[i_dk] += an * coeff_dk_an + bn * coeff_dk_bn + cn * coeff_dk_cn + dn * coeff_dk_dn;
			}
		}
	}
	DEBUG_END;

...
SContour::SContour() :
	id(-1), ns(0), plane(Plane(Vector3d(0, 0, 0), 0)), poly(NULL), sides(NULL)
{
	DEBUG_START;
	DEBUG_END;
}
...
std::vector<Vector3d> SContour::getPoints()
{
	...
	for (int iSide = 0; iSide < ns; ++iSide)
	{
		...
		DEBUG_PRINT("sides[%d]->A2 = (%lf, %lf, %lf)", iSidePrev,
					sidePrev->A2.x, sidePrev->A2.y, sidePrev->A2.z);
		DEBUG_PRINT("sides[%d]->A1 = (%lf, %lf, %lf)", iSide, sideCurr->A1.x,
					sideCurr->A1.y, sideCurr->A1.z);
...
SContour *SContour::convexify()
{
	...
	auto extremePoints = mapPointsFromOXYplane(
		hull, Vector_3(plane.norm.x, plane.norm.y, plane.norm.z));

...
SContour::SContour() : id(-1), ns(0), plane(Plane(Vector3d(0, 0, 0), 0)), poly(NULL), sides(NULL)
{
	DEBUG_START;
	DEBUG_END;
}

SContour::SContour(const SContour &orig) : id(orig.id), ns(orig.ns), plane(orig.plane), poly(orig.poly), sides()
{
	DEBUG_START;
...
SContour::SContour(std::vector<Point_3> points, Plane planeOrig) :
	id(-1), ns((int)points.size()), plane(planeOrig), poly(NULL), sides()
{
	DEBUG_START;
...
std::vector<Vector3d> SContour::getPoints()
{
	...
	for (int iSide = 0; iSide < ns; ++iSide)
	{
		...
		SideOfContour *sidePrev DEBUG_VARIABLE = &sides[iSidePrev];
		ASSERT(std::isfinite(sidePrev->A2.x));
		ASSERT(std::isfinite(sidePrev->A2.y));
		ASSERT(std::isfinite(sidePrev->A2.z));
		DEBUG_PRINT("sides[%d]->A2 = (%lf, %lf, %lf)", iSidePrev, sidePrev->A2.x, sidePrev->A2.y, sidePrev->A2.z);
		DEBUG_PRINT("sides[%d]->A1 = (%lf, %lf, %lf)", iSide, sideCurr->A1.x, sideCurr->A1.y, sideCurr->A1.z);
		...
		DEBUG_PRINT("   difference = (%lf, %lf, %lf)", diff.x, diff.y, diff.z);

		if (qmod(diff) >= EPS_MIN_DOUBLE)
		{
			vCurr = plane.project(sidePrev->A2);
			points.push_back(vCurr);
			DEBUG_PRINT(COLOUR_RED "Warning: contour %d is not connected! "
								   "A gap found between %d-th and %d-th sides." COLOUR_NORM,
						id, iSidePrev, iSide);
		}
	}
	DEBUG_END;
...
SContour *SContour::convexify()
{
	...
	convex_hull_2(pointsMapped.begin(), pointsMapped.end(), std::back_inserter(hull));

	if ((int)hull.size() != (int)points.size())
	{
		ASSERT(hull.size() < points.size());
		MAIN_PRINT(COLOUR_RED "Warning: contour #%d is non-convex, its hull "
							  "contains %d of %d of its points" COLOUR_NORM,
				   id, (int)hull.size(), (int)points.size());
	}

	/* TODO: Add automatic conversion from Vector3d to Vector_3 !!! */
	auto extremePoints = mapPointsFromOXYplane(hull, Vector_3(plane.norm.x, plane.norm.y, plane.norm.z));

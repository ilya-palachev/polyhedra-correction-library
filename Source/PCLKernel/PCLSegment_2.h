...
template <class R> inline bool PCLSegment_2<R>::operator==(const PCLSegment_2<R> &s) const
{
...
template <class R> CGAL_KERNEL_INLINE const typename PCLSegment_2<R>::Point_2 &PCLSegment_2<R>::vertex(int i) const
{
	return (i % 2 == 0) ? source() : target();
}

template <class R> CGAL_KERNEL_INLINE const typename PCLSegment_2<R>::Point_2 &PCLSegment_2<R>::point(int i) const
{
	return (i % 2 == 0) ? source() : target();
}

template <class R> inline const typename PCLSegment_2<R>::Point_2 &PCLSegment_2<R>::operator[](int i) const
{
	return vertex(i);
}

template <class R> CGAL_KERNEL_INLINE typename PCLSegment_2<R>::FT PCLSegment_2<R>::squared_length() const
{
	typename R::Compute_squared_distance_2 squared_distance;
	return squared_distance(source(), target());
}

template <class R> CGAL_KERNEL_INLINE typename PCLSegment_2<R>::Direction_2 PCLSegment_2<R>::direction() const
{
	typename R::Construct_vector_2 construct_vector;
	return Direction_2(construct_vector(source(), target()));
}

template <class R> CGAL_KERNEL_INLINE typename PCLSegment_2<R>::Vector_2 PCLSegment_2<R>::to_vector() const
{
	typename R::Construct_vector_2 construct_vector;
	return construct_vector(source(), target());
}

template <class R> inline typename PCLSegment_2<R>::Line_2 PCLSegment_2<R>::supporting_line() const
{
...
template <class R> inline typename PCLSegment_2<R>::Segment_2 PCLSegment_2<R>::opposite() const
{
...
template <class R> CGAL_KERNEL_INLINE bool PCLSegment_2<R>::is_horizontal() const
{
	return R().equal_y_2_object()(source(), target());
}
...
template <class R> CGAL_KERNEL_INLINE bool PCLSegment_2<R>::has_on(const typename PCLSegment_2<R>::Point_2 &p) const
{
	return R().collinear_are_ordered_along_line_2_object()(source(), p, target());
}

template <class R> inline bool PCLSegment_2<R>::collinear_has_on(const typename PCLSegment_2<R>::Point_2 &p) const
{
	return R().collinear_has_on_2_object()(*this, p);
}

template <class R> std::ostream &operator<<(std::ostream &os, const PCLSegment_2<R> &s)
{
...
template <class R> std::istream &operator>>(std::istream &is, PCLSegment_2<R> &s)
{

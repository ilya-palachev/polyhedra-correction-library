...
void EdgeList::search_and_set_info(
		int v0,
		int v1,
		int next_d,
		int next_f) {
...
void EdgeList::get_next_edge(
	...
	if (num < 1 && fabs(err) <= 0.0000000000000001) {
		...
		if (1) {
			...
			if (sign1 == 1) {
				//next_f = :
				poly->facets[next_f].find_next_facet(incr == 1 ?
						v0 : v1, next_f);
				next_d = 0;
			} else {
				next_d = incr;
